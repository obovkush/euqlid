window.addEventListener('DOMContentLoaded', function () {
  ...
  // Slider
  document
    .querySelectorAll('.section-work__nav-btn')
    .forEach(function (workBtn) {
      workBtn.addEventListener('click', function (event) {
        const path = event.currentTarget.dataset.path;

        document
          .querySelectorAll('.section-work__info')
          .forEach(function (workInfo) {
            workInfo.classList.remove('section-work__info-active');
          });
        document
          .querySelector(`[data-target="${path}"]`)
          .classList.add('section-work__info-active');
      });
    });
